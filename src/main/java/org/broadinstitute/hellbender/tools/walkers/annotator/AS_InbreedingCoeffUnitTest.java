...
public class AS_InbreedingCoeffUnitTest {
    ...
    public void testPerReadAlleleLikelihoodMap(){
        ...
        for (int i = 0; i < n1A; i++) {
            final GATKSAMRecord read = ArtificialSAMUtils.createArtificialRead(TextCigarCodec.decode("10M"));
        ...
        final Map<String, Object> annotate = new AS_RMSMappingQuality().annotateRawData(null, null, referenceContext, null, vc, perReadAlleleLikelihoodMap);
        ...
        Assert.assertEquals(annotate.get(GATKVCFConstants.AS_RAW_RMS_MAPPING_QUALITY_KEY), String.format("%.2f", rms));
    }
}
